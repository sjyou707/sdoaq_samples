...
namespace SDOAQ_App_CS
{
	public partial class SDOAQ_APP_CS : Form
	{
		...
		private IntPtr frmHandle = IntPtr.Zero;
		private bool frmclosing = false;
		private IntPtr m_hwnd3D = IntPtr.Zero;
        ...
        private Logger _myLogger = new Logger();
        private StringBuilder _logBuffer = new StringBuilder();
        private object _lockLogBuffer = new object();
        private Timer _tmrLogUpdate = new System.Windows.Forms.Timer();
		#endregion
		...
		public SDOAQ_APP_CS()
		{
            ...
            var rv = WSIORV.WSIORV_SUCCESS;

            foreach (var item in m_vhwndIV)
			{
                rv = UTIL.WSUT_IV_CreateImageViewer("Main viewer"
            ...
            rv = GL.WSGL_Initialize(this.pbViewer4.Handle, out m_hwnd3D);
            ...
            _myLogger.DataReceived += Logger_DataReceived;
            _tmrLogUpdate.Interval = 100;
            _tmrLogUpdate.Tick += tmrLogUpdate_Tick;
            _tmrLogUpdate.Start();
        }
        ...
        private void WriteLog(LogLevel logLevel, string format, params object[] args)
        {
            ...
            {
                _myLogger.WriteLog($"[{logLevel}] {format}");
            }
            else
            {
                _myLogger.WriteLog($"[{logLevel}] {string.Format(format, args)}");
            }   
        }

        private void tmrLogUpdate_Tick(object sender, EventArgs e)
        {
            ...
            lock (_lockLogBuffer)
            {
                rtbLOG.AppendText(_logBuffer.ToString());
		...
		unsafe public void OnReceiveZstack(eErrorCode eErrorCode, int lLastFilledRingBufferEntry)
		{
			if (eErrorCode.ecNoError != eErrorCode)
			{
				WriteLog(LogLevel.API, "SDOAQ_ContinuousAcquisitionCallback", eErrorCode);
                return;
			}

            if (SET.rb.active == false)
            {
                return;
            }
			...
			{
				WriteLog(LogLevel.Info, ">> SKIP to display {0} stacks.", lLastFilledRingBufferEntry);
			...
			for (uint uid = 0; uid < m_vhwndIV.Count; uid++)
			{
				if (uid < FOCUS.numsFocus)
				{
					...
					ImageViewer(uid, title, m_nContiStack, SET, SET.buffer[pos]);
				}
				else
				{
					ImageViewer(uid);
		...
		unsafe public void OnReceiveEdof(eErrorCode eErrorCode, int lLastFilledRingBufferEntry)
		{
			if (eErrorCode.ecNoError != eErrorCode)
			{
				WriteLog(LogLevel.API, "SDOAQ_ContinuousEdofCallback", eErrorCode);
			}
...
#else
                    byte[] bufferImgEdof = new byte[SET.ImgSize()];
                    Marshal.Copy(SET.rb.ppBuf[base_order + 0], bufferImgEdof, 0, bufferImgEdof.Length);
                    
                    ImageViewer(0, "EDoF", m_nContiEdof, SET, bufferImgEdof);
		...
		unsafe public void OnReceiveAF(eErrorCode eErrorCode, int lLastFilledRingBufferEntry, double dbBestFocusStep, double dbBestScore)
		{
			if (eErrorCode.ecNoError != eErrorCode)
			{
				WriteLog(LogLevel.API, "SDOAQ_ContinuousAfCallback", eErrorCode);
			}
			else if (SET.rb.active)
			{
				NativeMessage message = new NativeMessage();
...
#else
				ImageViewer(0, "AF", m_nContiAF, SET, SET.buffer[0]);
#endif
			}
		}
		...
		private void ReadySdoaqDll()
		{
			...
			WriteLog(LogLevel.Info, ">> DLL Version = {0}.{1}.{2}", nMajorVersion, nMinorVersion, nPatchVersion);
		}
			...
			if (eErrorCode.ecNoError == rv)
			{
				...
				if (flagAvailable)
				{
					...
					if (eType[0] == eParameterType.ptInt)
					{
						...
						{
							WriteLog(LogLevel.API, "[ParamID-{0}] has error!({1})", paraID , rv);
						}
					}
				...
				{
					WriteLog(LogLevel.API, "[ParamID-{0}] is not available for this product!({1})", paraID, rv);
				}
			}
		}
		...
		private void OnSdoaqSetParameter()
		{
			...
			{
				WriteLog(LogLevel.API, "OnSdoaqSetParameter: Parameter Not Selected!", eErrorCode.ecInvalidParameter);
				return;
			}
			...
			if (eErrorCode.ecNoError == rv)
			{
				...
				if (eErrorCode.ecNoError == rv)
				{
					...
					if (flagAvailable && flagWritable)
					{
						...
						if (eType[0] == eParameterType.ptInt)
						{
							...
							if (eErrorCode.ecNoError == rv)
							{
								...
								{
                                    WriteLog(LogLevel.API, "SDOAQ_SetIntParameterValue[ParamID-{0}] : value is out of range[{1} ~ {2}]({3})", paraID, nMin[0], nMax[0], eErrorCode.ecUnknownError);
								}
							}
							else
							{
                                WriteLog(LogLevel.API, "SDOAQ_GetIntParameterRange[ParamID-{0}]({1})", paraID, rv);
							}
						}
						else if (eType[0] == eParameterType.ptDouble)
						{
							...
							if (double.TryParse(sParameters, out dbValue))
							{
								...
								if (eErrorCode.ecNoError == rv)
								{
									...
									{
                                        WriteLog(LogLevel.API, string.Format("SDOAQ_SetDblParameterValue[ParamID-{0}] : value is out of range[{1:N1} ~ {2:N1}]", paraID, dbMin[0], dbMax[0]), eErrorCode.ecUnknownError);
									}
								}
								else
								{
                                    WriteLog(LogLevel.API, "SDOAQ_GetDblParameterRange[ParamID-{0}]({1})", paraID, rv);
								}
							}
							else
							{
                                WriteLog(LogLevel.API, "OnSdoaqSetParameter: Data should be of double type!({1})", eErrorCode.ecInvalidParameter);
							}
						}
						else // ptString
						{
					...
					else
					{
						if (!flagAvailable)
                        {
                            WriteLog(LogLevel.API, "OnSdoaqSetParameter: Not Available[ParamID-{0}]({1})", paraID, eErrorCode.ecInvalidParameter);
                        }

						if (!flagWritable)
                        {
                            WriteLog(LogLevel.API, "OnSdoaqSetParameter: Not Writable[ParamID-{0}]({1})", paraID, eErrorCode.ecInvalidParameter);
                        }
					}
				}
				else
				{
                    WriteLog(LogLevel.API, "SDOAQ_IsParameterWritable[ParamID-{0}]({1})", paraID, rv);
				}
			}
			else
			{
                WriteLog(LogLevel.API, "SDOAQ_IsParameterAvailable[ParamID-{0}]({1})", paraID, rv);
			}
		}
		...
		private void OnSdoaqSetROI()
		{
			...
			if (eErrorCode.ecNoError == rv)
			{
				...
				{
                    WriteLog(LogLevel.API, "Set cameraRoiLeft : value is out of range[{0} ~ {1}]({2})", nMin[0], nMax[0], eErrorCode.ecUnknownError);
					return;
				}
				if (AFP.cameraRoiWidth < 1 || AFP.cameraRoiWidth > nMax[0])
				{
                    WriteLog(LogLevel.API, "Set cameraRoiWidth : value is out of range[{0} ~ {1}]({2})", nMin[0], nMax[0], eErrorCode.ecUnknownError);
					return;
				}
			}
			...
			if (eErrorCode.ecNoError == rv)
			{
				...
				{
                    WriteLog(LogLevel.API, "Set cameraRoiTop : value is out of range[{0} ~ {1}]({2})", nMin[0], nMax[0], eErrorCode.ecUnknownError);
					return;
				}
				if (AFP.cameraRoiHeight < 1 || AFP.cameraRoiHeight > nMax[0])
				{
                    WriteLog(LogLevel.API, "Set cameraRoiHeight : value is out of range[{0} ~ {1}]({2})", nMin[0], nMax[0], eErrorCode.ecUnknownError);
					return;
				}
			}
		...
		private void OnSdoaqSetAFROI()
		{
			...
			if (eErrorCode.ecNoError != rv)
			{
                WriteLog(LogLevel.API, "set eParameterId.piFocusLeftTop({0})", rv);
			}
			rv = SDOAQ_SetIntParameterValue(eParameterId.piFocusRightBottom, (((nData[0] + nData[2]) & 0x0000FFFF) << 16) | ((nData[1] + nData[3]) & 0x0000FFFF) << 0);
			if (eErrorCode.ecNoError != rv)
			{
                WriteLog(LogLevel.API, "set eParameterId.piFocusRightBottom({0})", rv);
			}
		}
		...
		private void OnSdoaqSetEdofResize()
		{
			...
			if (double.TryParse(sEdofResize, out resize_ratio))
			{
				...
				if (eErrorCode.ecNoError == rv)
				{
					...
					{
                        WriteLog(LogLevel.API, "Set EDoF resize ratio : value is out of range[{0:N2} ~ {1:N2}]({2})", dbMin, dbMax, eErrorCode.ecUnknownError);
					}
				}
				else
				{
                    WriteLog(LogLevel.API, "SDOAQ_GetDblParameterRange [pi_edof_calc_resize_ratio]({0})", rv);
				}
			}
		}
		#endregion
		...
		unsafe private void OnSdoaqAcqStack()
		{
			...
			var pPositions = new int[FOCUS.numsFocus];
			var ppFocusImages = new IntPtr[FOCUS.numsFocus];
			var pFocusImageBufferSizes = new ulong[FOCUS.numsFocus];

			var size = SET.ImgSize();
			byte[][] bData = new byte[FOCUS.numsFocus][];

            for (int i = 0; i < bData.Length; i++)
            {
                bData[i] = new byte[size];
            }
			...
			try
			{
                ...
                {
                    fixed (byte* pointerToFirst = bData[pos])
                    {
                        pPositions[pos] = FOCUS.vFocusSet[pos];
                        ppFocusImages[pos] = new IntPtr(pointerToFirst);
                ...
                {
                    WriteLog(LogLevel.API, "SDOAQ_SingleShotFocusStack({0})", rv);
                }
			}
			catch (Exception ex)
			{
		...
		#region Image Viewer
		//----------------------------------------------------------------------------
		private void ImageViewer(uint uViewer, string title = null, int title_no = 0, SDOAQ_TEST_SET SET = null, byte[] data = null)
		{
			ImageViewer(uViewer, title, title_no, SET.afp.cameraRoiWidth, SET.afp.cameraRoiHeight, SET.COLORBYTES, data);
		}

		private void ImageViewer(uint uViewer, string title, int title_no, int width, int height, int colorbytes, byte[] data)
		{
				...
				if (cPictBox.InvokeRequired == true)
				{
					this.Invoke(new MethodInvoker(
						delegate ()
						{
							...
							{
                                WriteLog(LogLevel.Exception, (ex.ToString() + " " + title));
								//MessageBox.Show(ex.ToString(), "ERROR", MessageBoxButtons.OK, MessageBoxIcon.Error);
							}
						}));
				}
		...
		unsafe private void OnSdoaqContiStack()
		{
			...
			var size = SET.ImgSize();
			IntPtr[] ppFocusImages = new IntPtr[SET.rb.numsBuf];
			ulong[] pFocusImageBufferSizes = new ulong[SET.rb.numsBuf];

			SET.ClearBuffer();
			SET.AllocBuffer((int)SET.rb.numsBuf, size);
			...
			try
			{
				...
				{
                    fixed (byte* pointerToFirst = SET.buffer[pos])
                    {
                        ppFocusImages[pos] = new IntPtr(pointerToFirst);
                        pFocusImageBufferSizes[pos] = (ulong)size;
				...
				{
                    WriteLog(LogLevel.API, "SDOAQ_PlayFocusStack({0})", rv);
				}
			}
			catch (Exception ex)
			{
		...
		void OnSdoaqStopStack()
		{
			if (SET.rb.active)
			{
                ...
                System.Threading.ThreadPool.QueueUserWorkItem(StopStackThread, new object { });
			}
		...
		private void OnSdoaqAcqEdof()
		{
			...
			if (eErrorCode.ecNoError == rv)
			{
                ...
                FloatViewer(pStepMapImageBuffer != null && stepMapBufferSize > 0, 1, "StepMAP", m_nContiEdof, SET, pStepMapImageBuffer);
                FloatViewer(pHeightMapBuffer != null && heightMapBufferSize > 0, 2, "HeightMAP", m_nContiEdof, SET, pHeightMapBuffer);
			...
			{
                WriteLog(LogLevel.API, "SDOAQ_SingleShotEdof({0})", rv);
			}
		}
        ...
        unsafe private void FloatViewer(bool onoff, uint uViewer, string title, int title_no, SDOAQ_TEST_SET SET, void* data)
        {
            ...
            var ptrStart = (float*)data;
            var buffer = new float[pixels];
            ...
            FloatViewer(onoff, uViewer, title, title_no, SET, buffer);
        }
        //----------------------------------------------------------------------------
        private void FloatViewer(bool onoff, uint uViewer, string title, int title_no, SDOAQ_TEST_SET SET, float[] data)
		{
            if (onoff == false || data == null)
            {
                ImageViewer(uViewer, title, title_no);
			...
			var img = new byte[SET.PixelSize()];
			for (uint i = 0; i < pixels; i++)
			{
				img[i] = (byte)((data[i] - low) * inc);
			}

            ImageViewer(uViewer, title, title_no, SET.afp.cameraRoiWidth, SET.afp.cameraRoiHeight, SET.MONOBYTES, img);
		}
		#endregion
		...
		unsafe private void OnSdoaqContiEdof()
		{
			...
			{
                WriteLog(LogLevel.API, "SDOAQ_PlayEdof({0})", rv);
			}
		...
		private void OnSdoaqStopEdof()
		{
			if (SET.rb.active)
			{
                ...
                System.Threading.ThreadPool.QueueUserWorkItem(StopEdofThread, new object { });
			}
		}
		...
		private void OnSdoaqAcqAF()
		{
			...
			{
                WriteLog(LogLevel.API, "SDOAQ_SingleShotAF({0})", rv);
			}
		...
		unsafe private void OnSdoaqContiAF()
		{
			...
			try
			{
				...
				{
                    WriteLog(LogLevel.API, "SDOAQ_PlayAF({0})", rv);
				}
			}
			catch (Exception ex)
			{
		...
		private void OnSdoaqStopAF()
		{
			...
			{
				SET.rb.active = false;
                System.Threading.ThreadPool.QueueUserWorkItem(StopAFThread, new object { });
			}
		...
		private void btnSetCalibrationFile(object sender, EventArgs e)
		{
			...
			if (dlg.ShowDialog() == DialogResult.OK)
			{
                ...
                WriteLog(LogLevel.Info, ">> calibration data of file [{0}] is set.", dlg.FileName);
			}
		}
	}
}
